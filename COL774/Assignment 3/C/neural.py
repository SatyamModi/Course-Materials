...
class Nnet:
    ...
    def __init__(self, sizes, l_rate, batch_size, activation, activation_prime, epsilon=2e-5, decay=False):
...
_, train_path, test_path, output_path, question_part = sys.argv

train_data = pd.read_csv(train_path, delimiter=',').to_numpy()
test_data = pd.read_csv(test_path, delimiter=',').to_numpy()
...
# Implement a neural net with varying hidden layer units (sigmoid)
	...
	for i in range(len(hidden_layer_units)):
		n_unit = hidden_layer_units[i]
		network = Nnet([784, n_unit, 10], l_rate=0.1, batch_size=100, activation=sigmoid, activation_prime=sigmoid_prime)
		start = time.time()
		network.SGD(X_train, Y_train)
		end = time.time()

		train_acc_ = network.evaluate(X_train, Y_train)
		test_acc_ = network.evaluate(X_test, Y_test, h_units=str(n_unit), output_path=output_path, plot=True)
		time_taken.append(end-start)
		train_acc.append(train_acc_)
		test_acc.append(test_acc_)

# Implement a neural net with varying hidden layer units and decay learning rate(sigmoid)
	...
	for i in range(len(hidden_layer_units)):
		n_unit = hidden_layer_units[i]
		network = Nnet([784, n_unit, 10], l_rate=0.1, decay=True, batch_size=100, activation=sigmoid, activation_prime=sigmoid_prime)
		start = time.time()
		network.SGD(X_train, Y_train)
		end = time.time()
...
# Implement a relu and sigmoid on 2 hidden layers with 100 units
elif question_part == 'd':
	# For Relu activation
	network = Nnet([784, 100, 100, 10], l_rate=0.1, decay=True, batch_size=100, activation=relu, activation_prime=relu_prime)
	network.SGD(X_train, Y_train)
	train_acc_ = network.evaluate(X_train, Y_train)
	test_acc_ = network.evaluate(X_test, Y_test, title='relu', output_path=output_path, plot=True)
	print("Training accuracy on relu: {}".format(train_acc_))
	print("Test accuracy on relu: {}".format(test_acc_))

	# For Sigmoid activation
	network = Nnet([784, 100, 100, 10], l_rate=0.1, decay=True, batch_size=100, activation=sigmoid, activation_prime=sigmoid_prime)
	network.SGD(X_train, Y_train)
	train_acc_ = network.evaluate(X_train, Y_train)
	test_acc_ = network.evaluate(X_test, Y_test, title='sigmoid', output_path=output_path, plot=True)
	print("Training accuracy on sigmoid: {}".format(train_acc_))
	print("Test accuracy on sigmoid: {}".format(test_acc_))
...
# keeping number of units to be 50 in each layer
	...
	relu_test_acc = []
	for i in range(2, 6):
		network = Nnet([784]+[50]*i+[10], l_rate=0.1, batch_size=100, activation=relu, activation_prime=relu_prime)
		network.SGD(X_train, Y_train)
		train_acc_ = network.evaluate(X_train, Y_train)
	...
	sigmoid_test_acc = []
	for i in range(2, 6):
		network = Nnet([784]+[50]*i+[10], l_rate=0.1, batch_size=100, activation=sigmoid, activation_prime=sigmoid_prime)
		network.SGD(X_train, Y_train)
		train_acc_ = network.evaluate(X_train, Y_train)
		test_acc_ = network.evaluate(X_test, Y_test)
	...
	network = Nnet([784, 50, 50, 10], l_rate=0.1, epsilon=4e-6, batch_size=100, activation=relu, activation_prime=relu_prime)
	network.SGD(X_train, Y_train)
	train_acc_ = network.evaluate(X_train, Y_train)
	test_acc_ = network.evaluate(X_test, Y_test, title='relu', output_path=output_path, plot=True)
	print("Training accuracy on relu: {}".format(train_acc_))
