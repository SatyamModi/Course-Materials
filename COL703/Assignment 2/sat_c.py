...
def proof_check(i):
    ...
    if i == len(p_lines):
        # check whether the last clause in the p_clause is empty
            ...
            if (atoms != '??'):
                ...
                for j in range(num_formulas):
                    ...
                    if r_clause != NOT_RESOLVED:
                        ...
                        print("i: {}, atoms: {}".format(i, atoms))
                        print("p_clauses: ", p_clauses)
                        print("chosen clause: ", blanks[-1])
...
def solve(proof_filename, formula_filename):
    global p_lines
    global num_atoms
    global num_formulas
    global f_clauses
    ...
    print("p_lines: ", p_lines)
    print("f_clauses: ", f_clauses)
    print("num_atoms: ", num_atoms)
    print("num_formulas: ", num_formulas)
    return proof_check(0)

proof_filename = sys.argv[2]
formula_filename = sys.argv[1]
result = solve(proof_filename, formula_filename)
print("result: ", result)
if (result == 1):
    idx = 0
    for i in range(len(p_lines)):
